...
static int parse_http_rsp(char *buf, size_t len, netw_http_rsp_t *rsp) {
    char *http;
    char *body;
...
static int recv_all(int sock, char *buf, size_t buf_len) {
    int ret;
    int len = 0;
    while (len < (buf_len - 1)) {
        ret = zsock_recv(sock, buf + len, buf_len - len - 1, 0);
        if (ret > 0) {
            len += ret;
            continue;
        }
...
int netw_http_get( const char *ip, uint16_t port, const char *host, const char *path, char *buf, size_t buf_len, netw_http_rsp_t *rsp) {
    int ret = 0;
    int sock;
    ...
    do {
        ...
        snprintk(req, sizeof(req), "GET %s HTTP/1.1\r\n" "Host: %s\r\n" "Connection: close\r\n\r\n", path, host);

        if (send_all(sock, req, strlen(req)) < 0) {
            break;
        }

        if (recv_all(sock, buf, buf_len) < 0) {
            ret = -errno;
            break;
        }
        ret = parse_http_rsp(buf, ret, rsp);
    } while (0);
    zsock_close(sock);
    return ret;
}